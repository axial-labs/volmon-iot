...
void setup()
{
  ...
  if (settings.begin("settings", true) == false)
  {
    settings.begin("settings", false);
  }
  memset(ssid, 0, sizeof(ssid));
  memset(password, 0, sizeof(password));
  settings.getString("SSID", "").toCharArray(ssid, sizeof(ssid));
  settings.getString("Password", "").toCharArray(password, sizeof(password));
  settings.end();
...
void buttonCall(Control *sender, int type)
{
  if (type == B_DOWN)
  {
    ...
    else if (sender->id == saveConfigButtonId)
    {
      ...
      settings.begin("settings", false);
      settings.putString("SSID", String(temp_ssid));
      settings.putString("Password", String(temp_password));
      settings.end();
